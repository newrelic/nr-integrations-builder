...
func TestCreateLocalEntity(t *testing.T) {
	i, err := integration.New("test", "1.0.0")
	assert.NoError(t, err)
	...
	assert.Equal(t, `{"name":"test","protocol_version":"2","integration_version":"1.0.0","data":[{"metrics":[],"inventory":{},"events":[]}]}`, string(j))
...
func TestCreateLocalEntityWithInventory(t *testing.T) {
	i, err := integration.New("test", "1.0.0")
	assert.NoError(t, err)

	e := i.LocalEntity()

	e.Inventory.SetItem("foo", "bar", "quux")

	ii, exists := e.Inventory.Item("foo")
	assert.Equal(t, exists, true)
	assert.Equal(t, "quux", ii["bar"])

	assert.Equal(t, len(e.Inventory.Items()), 1)
